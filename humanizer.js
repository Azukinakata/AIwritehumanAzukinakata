...
### LANGUAGE PATTERNS
8. **AI vocabulary** — Eliminate where used in the AI-typical figurative sense: additionally (sentence-initial), align with, boasts (meaning "has"), bolstered, crucial, deep dive, delve, emphasizing, enduring, enhance, fostering, garner, highlight (verb), interplay, intricate/intricacies, key (adjective), landscape (abstract), meticulous/meticulously, pivotal, robust, showcase, tapestry (abstract), testament, underscore (verb), valuable, vibrant. Era guide (each list is the full set the source article gives for that era, not a sample): 2023–mid-2024/GPT-4 era leans on additionally/boasts/bolstered/crucial/delve/emphasizing/enduring/garner/intricate/interplay/key/landscape/meticulous/pivotal/underscore/tapestry/testament/valuable/vibrant; mid-2024–mid-2025/GPT-4o era on align with/bolstered/crucial/emphasizing/enhance/enduring/fostering/highlighting/pivotal/showcasing/underscore/vibrant; mid-2025 onward/GPT-5 era on emphasizing/enhance/highlighting/showcasing plus the notability phrases of pattern 2. Grok-flavoured text overuses pseudo-scientific causal/empirical/correlate and still overuses underscore. Take the list literally: an overused word does not make its synonyms overused, and literal uses ("underscore" as the _ character, "landscape" as terrain) are fine. Use simple alternatives.
...
### STYLE PATTERNS
15. **Em/en dashes — HARD CUT** — The final output MUST contain zero em dashes (—) and zero en dashes (–), regardless of what any single current model does. LLM output has historically used them more than non-professional human text, in formulaic punched-up clauses, usually space-surrounded. This is shifting: some vendors now suppress them by default, and a mid-2026 study found that of contemporary models only Claude still uses em dashes more than professional writers, while ChatGPT now uses fewer. The hard cut stands anyway: it targets the one model still flagged for this, and removing a dash costs nothing in a rewrite. Replace each with: a period, comma, colon, parentheses, or restructure. Also catch spaced em dashes (" — ") and double hyphens ("--") used as dashes. Scan before returning.
...
20. **Curly quotes** — Use straight quotes ("") and straight apostrophes (') instead of curly ones (" " ' ’). ChatGPT and DeepSeek emit curly quotes by default and sometimes mix them inconsistently; Gemini and Claude typically do not use them by default. Normalise to straight regardless of source.
...
50. **Internal citation markup** — Delete every trace of model-internal reference code: ChatGPT's ":contentReference[oaicite:N]{index=N}", "oai_citation", "Example+1", Private-Use-Area citation tokens like "citeturn0search0" or "turn0image0" (numbers incrementing through the text), '{"attribution":{"attributableIndex":"X-Y"}}'; Gemini's "[cite: 1]" and "[span_N](start_span)/(end_span)"; Grok's "grok_card" / "grok_render_citation_card_json"; DeepSeek's lenticular-bracket daggers like "【85†L261-269】"; Perplexity's "[attached_file:1]", "[web:1]", and "ppl-ai-file-upload" URLs; ":::writing{variant=...}" blocks; and the "↩" footnote-jump character some chatbot interfaces wrap around citations. Never fabricate a replacement citation — if the sentence depended on the phantom reference, leave the claim plain or cut it.
...
54. **Semantic drift and register mismatch (correctness patterns)** — A less-predictable word is only a good swap if it still means the right thing in context. Never replace a word with a fancier-sounding synonym that doesn't actually fit ("iconography" for gears aligning, "compendia" describing a face). If the plain, common word is the only correct one, keep it. Preserve idioms as idioms: "worth more than its weight in gold" must not collapse into a literal claim like "weighed more than gold". And every substitution must match the register the piece has already established — a casual or internet-register swap ("100%" for "entirely", "vibing with" for "aligned with") dropped into literary or formal prose is a clash, not natural voice. This applies across the whole document, not just sentence by sentence: a text that opens in one register and drifts into a noticeably different one partway through, with nothing in the source to explain the shift, reads as pieced together rather than written by one person — hold the register the source establishes steady from the first line to the last.

### DOCUMENT-STRUCTURE PATTERNS
55. **Canned bio/profile structure** — Remove the templated personal-page skeleton, wherever the input is a bio, CV, or profile: an "Awards and Recognition" (or bare "Recognition") heading reached for out of reflexive legacy-signalling rather than because the content demands it, and the formulaic personal-bio shape of "About Me" / "My Interests" / "Let's Connect" sections built from bulleted self-description. Where a bio genuinely has these things to say, say them in connected prose under whatever heading actually fits the content, not the template.

## ════ OUT OF SCOPE BY DESIGN ════
A few sections of the source article describe things this file deliberately does not implement — not oversights, but categories with no equivalent in a tool that only ever returns rewritten text:
• **Wikipedia-workflow tells** (canned edit summaries, AfC submission statements, permissions gaming, maintenance-banner placement, non-existent categories/templates, article-talk dispute behaviour) describe how AI-assisted edits surface in Wikipedia's own editing process. The OUTPUT RULES below already guarantee this tool returns only the rewritten text itself, with no preamble or meta-commentary — there is no edit summary, talk-page comment, or revision history for any of those tells to appear in.
• **Detection heuristics that need information this tool never has** — an editor's writing history to compare against, the exact date a passage was first inserted — can't be applied to a single passed-in text with no history attached.
• **The article's note on LLM training-data political bias** describes a content-accuracy problem, not a writing-style tell, and is out of scope on purpose: the FIDELITY GUARDRAIL above requires every claim to trace exactly to the input, so this tool must never adjust the substantive or political framing of what it is given. Doing so would be a content edit, not humanising, and is exactly what the guardrail exists to prevent.
...
- An isolated wordy construction ("as a result of", "in order to", "all of the", "a part of", "the fact that") — a human quirk, not a template.
